...
- GPT 인증 상태 확인 및 안내 (API 키 → codex CLI → auth.json 순)
...
자동 시도됩니다. 서버 환경에서 `OPENAI_API_KEY`나 `codex` CLI 로그인을 쓸 수 없다면 마지막 수단으로
OAuth 방식(`~/.codex/auth.json`)을 씁니다. 브라우저가 없는 서버에서는 다른 머신에서 파일을 복사합니다.
...
│       ├── write-guard.js       # 기존 파일 덮어쓰기 방지 (PreToolUse, advisory)
...
```
/hard <작업 설명>
또는 메시지에 hardmode / 하드모드 키워드 포함 (ulw-detector가 감지해 자동 주입)
```
...
규칙(전체 완료+검증 통과 전 종료 금지). 중지는 `/stop-continuation`. 토큰/비용 소모가 크므로
일반 작업에는 쓰지 않는다.
...
| `write-guard` | PreToolUse | 기존 파일 덮어쓰기 전 Read 여부 확인, 미확인 시 경고 — **권고성(advisory) 훅, 실제 차단은 하지 않음** |
...
| `ulw-detector` | UserPromptSubmit | ULW/HARD 모드 키워드 감지 후 프로토콜 주입 |
...
# GPT auth — 위 OPENAI_API_KEY/codex CLI가 모두 없을 때만 이 auth.json(3순위, chatgpt_oauth)이 쓰임
python3 -c "
import json
...
| Activity Hooks | `hooks/` 디렉토리 | pre-call-indicator, post-call-logger, agent-banner [NEW v6.0], comment-checker, write-guard(advisory) |
