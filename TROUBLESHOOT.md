...
> `install.sh`와 `auth-setup.js`가 MCP env를 저장하며, 실패 시 settings.json 직접 편집으로 폴백함.
...
- [ ] `bash claude-omo/install.sh` 또는 `node ~/mcp-servers/multi-model/auth-setup.js`로 API 키 입력
